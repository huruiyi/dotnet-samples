...
namespace SSHOP
{
    public class DisplayGoodsHandler : IHttpHandler
    {
        ...
        {
            string html = File.ReadAllText(context.Server.MapPath("~/htmls/DisplayGoodsTemp.htm"));
            ...
            foreach (string gt in goodsType)
            {
                ...
                for (int i = 0; i < pros.Count; i++)
                {
                    ...
                    {
                        sb.Append("<td>");
                        sb.AppendFormat("<a href='BuyProduct.html?pid={0}'>", pros[i].ProductID);
                ...
                for (int i = 0; i < pros.Count; i++)
                {
                    if (gt == pros[i].Producttype)
                    {
                        sb.AppendFormat("<td align='center'>{2}&nbsp;&nbsp;<a href='BuyProduct.html?pid={1}'>{0}</a></td>", pros[i].ProductName, pros[i].ProductID, pros[i].UnitPrice.ToString("F2"));
                    }
                }
                sb.Append("</tr>");
                sb.Append("</table>");
