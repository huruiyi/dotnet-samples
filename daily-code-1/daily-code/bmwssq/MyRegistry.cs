...
namespace bmwssq
{
    public class MyRegistry
    {
        ...
        {
            string registryASCII ="";
            string ascii = GetDiskVolumeSerialNumber() + GetCpu();
            registryASCII = regInPassword(ascii);
        ...
        public int setRegedit()
        {
            ...
            try
            {
                res = Registry.LocalMachine.OpenSubKey(@"SOFTWARE\BMW");
                if (res == null)
                {
                    res = Registry.LocalMachine.CreateSubKey(@"SOFTWARE\BMW");
